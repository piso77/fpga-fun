...
DisplayLoop:			; read horizontal paddle position
...
MaxSpeed:
	...
	ldb	#TRACKPOS_LO	; add to lo byte of track pos
	...
	ldb	#TRACKPOS_HI	; add to hi byte of track pos
	...
	swapab				; swap A <-> B again
NoCarry:
	ldb	#ENEMY_Y		; update enemy vert pos
	add	A,[B]
	sta	ENEMY_Y			; enemy_y = enemy_y + speed/16
	ldb	#ENEMY_X		; update enemy horiz pos
	mov	A,[B]
	ldb	#ENEMY_DIR
	add	A,[B]
	...
	zero A				; load ENEMY_DIR and negate
	sub	A,[B]
	sta	ENEMY_DIR		; enemy_dir = -enemy_dir
SkipXReverse:
	jmp	DisplayLoop		; back to display loop
